...
const {verifyClassName, verifyWhirlerProperties} = require('../validation/verifyWhirler');
...
class Whirler extends WhirlerCore {
    ...
    async get(content) {
        ...
        const descriptor = Object.getOwnPropertyDescriptor(User, content.getter);
    ...
    async set(content) {
        ...
        const descriptor = Object.getOwnPropertyDescriptor(User, content.setter);
        if (descriptor !== undefined) {
            if(descriptor.enumerable && (descriptor.set != undefined ||
                (!(descriptor.value instanceof Function) && descriptor.writable))) {
