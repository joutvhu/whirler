...
function convertGetRequset(body) {
    ...
    return new Content(SystemConstants.GET_REQUEST, body.get);
}
...
function convertPutRequset(body) {
    ...
    return new Content(SystemConstants.SET_REQUEST, body.set, body.val);
}
...
function convertPostRequset(body) {
    ...
    if (body.args instanceof Array && body.args.length > 0)
        return new Content(SystemConstants.CALL_REQUEST, body.func, body.args);
    else return new Content(SystemConstants.CALL_REQUEST, body.func);
}
...
function convertWhirlerArray(whirles) {
    ...
    for (i of whirles) {
        if (i.prototype instanceof WhirlerCore && i.name) {
            if (!result[i.name])
                result[i.name] = i;
            else throw new DuplicationError(StringType.replacePlaceholders(ExceptionMessages.DUPLICATE_WHIRLER, {
                WHIRLERNAME: i.name
            }));
        } else throw new FormattedError(ExceptionMessages.SURE_WHIRLER);
    }
