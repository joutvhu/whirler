...
function preventOverrideClass(sighClass, sighObj, except) {
  ...
  if (sighObj && sighObj['__proto__'] instanceof sighClass) {
    ...
    if (error) throw new _OtherErrors.OverridingError(_ExceptionMessages.default.OVERRIDE_CLASS.replace('[ClassName]', sighClass.name));
  }
}
...
function preventOverrideFunction(sighClass, functions, sighObj) {
  ...
  while (obj instanceof sighClass) {
    for (i of functions) {
      if (typeof i === 'string' && obj.hasOwnProperty(i)) throw new _OtherErrors.OverridingError(_ExceptionMessages.default.OVERRIDE_FUNCTION.replace('[FunctionName]', i).replace('[ClassName]', sighClass.name));
    }

    if (sighObj['__proto__']) obj = obj['__proto__'];else break;
  }
}