...
test('call function and expect exception', () => {
    ...
    expect(whirlerObj.call({ function: 'getError' })).rejects.toThrow(stringConstants.error);
    expect(whirlerObj.call({ function: 'getError', arguments: [] })).rejects.toThrow(stringConstants.error);
});

test('call the not exist function', () => {
    const whirlerObj = new WhirlerC();

    expect(whirlerObj.call({ function: 'notExist' })).rejects.toThrow(ErrorMessages.FUNCTION_NOT_EXIST);
    expect(whirlerObj.call({ function: 'not Exist', arguments: [] })).rejects.toThrow(ErrorMessages.FUNCTION_NOT_EXIST);
});